...
const BookContents = ({route, navigation}) => {
  ...
  const {selected_Book_Data} = useSelector((state) => state.BookList);

  const [ValueTitle, setValueTitle] = useState(selected_Book_Data.bookName);
  const [ValueContent, setValueContent] = useState(
    selected_Book_Data.bookRecord,
  );
  ...
  const momo = () => {
    realm.write(() => {
      ...
      let user = realm.create(
        'User',
        {
          createtime: selected_Book_Data.createtime,
        },
        true,
      );
  ...
  navigation.addListener('blur', () => {
    if (
      selected_Book_Data.bookName != ValueTitle ||
      selected_Book_Data.bookRecord != ValueContent
    ) {
      navigation.navigate('My List', {
        bookReTitle: ValueTitle,
        post: ValueContent,
        time: selected_Book_Data.createtime,
      });
    }
  });
    ...
    <Container>
      ...
      <BookMark_FlatList>
        <FlatList
          ...
          data={selected_Book_Data.bookSentence}
...
import realm from '../../db';
import {MY_BOOKLIST_DATA, TEST_DATA_TEST} from '../../reducers/BookList';
import HelloTest from './HelloTest';
import {User} from 'realm';
import {useTheme} from '@react-navigation/native';
...
const BookContents = ({route, navigation}) => {
  const dispatch = useDispatch();
  const {colors} = useTheme();
  const {selected_Book_Data, user_book_data} = useSelector(
    (state) => state.BookList,
  );
  const [ValueTitle, setValueTitle] = useState(
    selected_Book_Data.item.bookName,
  );
  const [ValueContent, setValueContent] = useState(
    selected_Book_Data.item.bookRecord,
  );
  const [isModalVisible, setModalVisible] = useState(false);
  const [bookMarkeContent, setBookMarkeContent] = useState('');
  ...
  const momo = () => {
    realm.write(() => {
      ...
      let user = realm.create(
        'User',
        {
          createtime: selected_Book_Data.item.createtime,
        },
        true,
      );
      user.bookSentence.push(city);
  ...
  const book_Delete = () => {
    const BookAllData = realm.objects('User');
    const BookFilter = BookAllData.filtered(
      'bookName == $0',
      selected_Book_Data.item.bookName,
    );
    ...
    const SortBookDate = BookAllData.sorted('createtime');
  ...
  navigation.addListener('blur', () => {
    if (
      selected_Book_Data.item.bookName != ValueTitle ||
      selected_Book_Data.item.bookRecord != ValueContent
    ) {
      navigation.navigate('My List', {
        bookReTitle: ValueTitle,
        post: ValueContent,
        time: selected_Book_Data.item.createtime,
      });
    }
  });
  ...
  return (
    <Container>
      ...
      <BookMark_FlatList>
        <FlatList
          keyExtractor={(item, index) => '#' + index}
          data={selected_Book_Data.item.bookSentence}
          renderItem={(item) => <HelloTest hello={item} />}
        />
      </BookMark_FlatList>
    </Container>
